function onTimeForExam(input) {
  ...
  if (timeOfCooming > timeOfExam) {
    ...
    if (timeOfCooming - timeOfExam < 60) {
      ...
      console.log(
        `${Math.floor((timeOfCooming - timeOfExam) / 60)}:${
          (timeOfCooming - timeOfExam) % 60
        }  hours after the start`
      );
    } else if ((timeOfCooming - timeOfExam) % 60 < 10) {
      console.log(
        `${Math.floor((timeOfCooming - timeOfExam) / 60)}:0${
          (timeOfCooming - timeOfExam) % 60
        }  hours after the start`
      );
    }
    ...
    if (timeOfExam - timeOfCooming < 60) {
      ...
      console.log(
        `${Math.floor((timeOfExam - timeOfCooming) / 60)}:${
          (timeOfExam - timeOfCooming) % 60
        }  hours before the start`
      );
    } else if ((timeOfExam - timeOfCooming) % 60 < 10) {
      console.log(
        `${Math.floor((timeOfExam - timeOfCooming) / 60)}:0${
          (timeOfExam - timeOfCooming) % 60
        }  hours before the start`
      );
    }
